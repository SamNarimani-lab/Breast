...
class UNetPlusPlusASPP(nn.Module):
    def __init__(self , spatial_dims , in_channels , out_channels , features , dropout ):
        super(ModefiedBasicUNetPlusPlus , self). __init__()
        self.modefiedUNetPlusPlus = BasicUNetPlusPlus (spatial_dims= spatial_dims, in_channels=in_channels , out_channels=in_channels , features=features
                                       , act=('LeakyReLU', {'inplace': True, 'negative_slope': 0.1}),
                                       norm=('instance', {'affine': True}), bias=True, dropout= dropout , upsample='deconv')
        self.ASPP = ASPP(in_channels = in_channels , out_channels = out_channels , num_filters=2)
