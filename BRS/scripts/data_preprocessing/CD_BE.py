...
class CustomDataForBreastpp (Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.oversample :
            ...
            for i in range(pre_image.shape[-1]):
                ...
                # min_pre = pre_img.min()
                # max_pre = pre_img.max()
                # pre_img = (pre_img - min_pre) / (max_pre - min_pre)
                ...
                # min_post = post_img.min()
                # max_post = post_img.max()
                # post_img = (post_img - min_post) / (max_post - min_post)
...
class CustomDataForBreast (Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input = torch.tensor(pre_contrast_data).unsqueeze(1)
